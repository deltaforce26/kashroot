...
def test_migration_chain_heads_at_0008():
    ...
    assert script.get_heads() == ["0009_sheet_sync_proposal"]
    assert (
        script.get_revision("0009_sheet_sync_proposal").down_revision
    ...
    assert (
        script.get_revision("0006_drop_certifier_parent_id").down_revision
        == "0005_certificate_is_demo_seed"
    )
    assert (
