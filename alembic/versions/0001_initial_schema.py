...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE diet_type AS ENUM ('meat', 'dairy', 'pareve', 'fish', 'mixed', 'dairy_pareve')"
    )
    op.execute("CREATE TYPE restaurant_status AS ENUM ('open', 'closed_temp', 'closed_perm')")
    op.execute(
        "CREATE TYPE record_state AS ENUM ('list_verified', 'moderator_verified', 'owner_submitted', 'field_verified', 'unknown_pending_verification')"
    )
    op.execute(
        "CREATE TYPE certifier_type AS ENUM ('rabbanut_local', 'rabbanut_national', 'badatz', 'private')"
    )
    op.execute("CREATE TYPE certification_level AS ENUM ('unknown', 'regular', 'mehadrin')")
    op.execute(
        "CREATE TYPE certificate_source AS ENUM ('certifier_portal', 'official_list', 'moderator_verified', 'owner_submitted', 'field_verification')"
    )
    op.execute("CREATE TYPE certificate_state AS ENUM ('active', 'expired', 'revoked', 'pending')")
    op.execute(
        "CREATE TYPE hours_rule_type AS ENUM ('weekly', 'erev_shabbat', 'shabbat', 'erev_chag', 'chag', 'chol_hamoed')"
    )
    op.execute(
        "CREATE TYPE photo_kind AS ENUM ('storefront', 'interior', 'food', 'menu', 'certificate')"
    )
    op.execute(
        "CREATE TYPE flag_type AS ENUM ('closed', 'no_certificate_displayed', 'different_certifier', 'expired_certificate', 'wrong_details', 'wrong_hours', 'other')"
    )
    op.execute("CREATE TYPE flag_state AS ENUM ('open', 'in_review', 'resolved', 'rejected')")
    op.execute(
        "CREATE TYPE owner_claim_state AS ENUM ('pending', 'approved', 'rejected', 'revoked')"
    )
    op.execute("CREATE TYPE user_role AS ENUM ('user', 'owner', 'moderator', 'admin')")
    op.execute("CREATE TYPE language AS ENUM ('he', 'en')")
    op.execute(
        "CREATE TYPE source_document_kind AS ENUM ('pdf', 'image', 'web', 'api', 'portal', 'manual')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "type", postgresql.ENUM(name="certifier_type", create_type=False), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["parent_id"],
            ["certifier.id"],
            name=op.f("fk_certifier_parent_id_certifier"),
            ondelete="SET NULL",
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "state",
            postgresql.ENUM(name="ingestion_run_state", create_type=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(name="restaurant_status", create_type=False),
            server_default="open",
            nullable=False,
        ),
        sa.Column(
            "record_state", postgresql.ENUM(name="record_state", create_type=False), nullable=False
        ),
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_restaurant_price_level_range"),
        ),
    ...
    op.create_index(
        "ix_restaurant_geo", "restaurant", ["geo"], unique=False, postgresql_using="gist"
    )
    ...
    op.create_index(
        op.f("ix_restaurant_needs_review"), "restaurant", ["needs_review"], unique=False
    )
    op.create_table(
        "audit_log",
        ...
        sa.Column(
            "action", postgresql.ENUM(name="audit_action", create_type=False), nullable=False
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["ingestion_run_id"],
            ["ingestion_run.id"],
            name=op.f("fk_audit_log_ingestion_run_id_ingestion_run"),
            ondelete="SET NULL",
        ),
    ...
    op.create_index(
        op.f("ix_audit_log_ingestion_run_id"), "audit_log", ["ingestion_run_id"], unique=False
    )
    op.create_table(
        "opening_hours",
        sa.Column("restaurant_id", sa.UUID(), nullable=False),
        sa.Column(
            "rule_type", postgresql.ENUM(name="hours_rule_type", create_type=False), nullable=False
        ),
        ...
        sa.CheckConstraint(
            "(rule_type = 'weekly' and weekday is not null) or (rule_type <> 'weekly' and weekday is null)",
            name=op.f("ck_opening_hours_weekday_only_for_weekly"),
        ),
        sa.CheckConstraint(
            "weekday is null or weekday between 0 and 6",
            name=op.f("ck_opening_hours_weekday_range"),
        ),
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_opening_hours_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_opening_hours")),
    )
    op.create_index(
        "ix_opening_hours_restaurant_id_rule_type",
        "opening_hours",
    ...
    op.create_table(
        ...
        sa.Column(
            "state",
            postgresql.ENUM(name="owner_claim_state", create_type=False),
        ...
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_owner_claim_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["reviewed_by_user_id"],
            ["app_user.id"],
            name=op.f("fk_owner_claim_reviewed_by_user_id_app_user"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["app_user.id"],
            name=op.f("fk_owner_claim_user_id_app_user"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_owner_claim")),
    )
    op.create_index(
        op.f("ix_owner_claim_restaurant_id"), "owner_claim", ["restaurant_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_restaurant_photo_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["uploaded_by_user_id"],
            ["app_user.id"],
            name=op.f("fk_restaurant_photo_uploaded_by_user_id_app_user"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_restaurant_photo")),
    )
    op.create_index(
        op.f("ix_restaurant_photo_restaurant_id"),
        "restaurant_photo",
        ["restaurant_id"],
        unique=False,
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["app_user.id"],
            name=op.f("fk_saved_list_user_id_app_user"),
            ondelete="CASCADE",
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "kind", postgresql.ENUM(name="source_document_kind", create_type=False), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["certifier_id"],
            ["certifier.id"],
            name=op.f("fk_source_document_certifier_id_certifier"),
            ondelete="SET NULL",
        ),
    ...
    op.create_index(
        op.f("ix_source_document_certifier_id"), "source_document", ["certifier_id"], unique=False
    )
    op.create_table(
        "user_profile",
        sa.Column("user_id", sa.UUID(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "level",
            postgresql.ENUM(name="certification_level", create_type=False),
        ...
        sa.Column(
            "state", postgresql.ENUM(name="certificate_state", create_type=False), nullable=False
        ),
        sa.Column(
            "source", postgresql.ENUM(name="certificate_source", create_type=False), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["certifier_id"],
            ["certifier.id"],
            name=op.f("fk_certificate_certifier_id_certifier"),
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_certificate_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["source_document_id"],
            ["source_document.id"],
            name=op.f("fk_certificate_source_document_id_source_document"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["verified_by_user_id"],
            ["app_user.id"],
            name=op.f("fk_certificate_verified_by_user_id_app_user"),
            ondelete="SET NULL",
        ),
    ...
    op.create_index(
        "ix_certificate_certifier_id_state", "certificate", ["certifier_id", "state"], unique=False
    )
    op.create_index(
        "ix_certificate_restaurant_id_state",
        "certificate",
        ["restaurant_id", "state"],
        unique=False,
    )
    op.create_index(
        op.f("ix_certificate_source_document_id"),
        "certificate",
        ["source_document_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["certifier_id"],
            ["certifier.id"],
            name=op.f("fk_profile_certifier_whitelist_certifier_id_certifier"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["profile_id"],
            ["user_profile.id"],
            name=op.f("fk_profile_certifier_whitelist_profile_id_user_profile"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint(
            "profile_id", "certifier_id", name=op.f("pk_profile_certifier_whitelist")
        ),
    )
    op.create_table(
        "saved_list_item",
        ...
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_saved_list_item_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["saved_list_id"],
            ["saved_list.id"],
            name=op.f("fk_saved_list_item_saved_list_id_saved_list"),
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        op.f("ix_saved_list_item_restaurant_id"), "saved_list_item", ["restaurant_id"], unique=False
    )
    op.create_index(
        op.f("ix_saved_list_item_saved_list_id"), "saved_list_item", ["saved_list_id"], unique=False
    )
    op.create_table(
        "flag",
        sa.Column("restaurant_id", sa.UUID(), nullable=False),
        ...
        sa.Column(
            "state",
            postgresql.ENUM(name="flag_state", create_type=False),
            server_default="open",
            nullable=False,
        ),
        ...
        sa.ForeignKeyConstraint(
            ["certificate_id"],
            ["certificate.id"],
            name=op.f("fk_flag_certificate_id_certificate"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["resolved_by_user_id"],
            ["app_user.id"],
            name=op.f("fk_flag_resolved_by_user_id_app_user"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["restaurant_id"],
            ["restaurant.id"],
            name=op.f("fk_flag_restaurant_id_restaurant"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["user_id"], ["app_user.id"], name=op.f("fk_flag_user_id_app_user"), ondelete="SET NULL"
        ),
